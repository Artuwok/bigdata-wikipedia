...
object WikipediaRanking {
  ...
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int = {
    val wordsRdd: Int = wikiRdd.filter(_.text.contains(lang))
      ...
      .count().toInt
    // val count: RDD[(String, Int)] = wordsRdd.filter(_.equalsIgnoreCase(lang)).map(w => (w, 1)).reduceByKey { case (x, y) => x + y }
    // count.count().toInt
    wordsRdd
  }
  ...
  def makeIndex(langs: List[String], rdd: RDD[WikipediaArticle]): RDD[(String, Iterable[WikipediaArticle])] = {
    for (lang <-langs){
      val v: RDD[WikipediaArticle] =  rdd.filter(_.text.contains(" " + lang + " "))
      (lang, v.collect().toList)
  }
  ...
  def main(args: Array[String]) {
   ...
   println("Res: " + occurrencesOfLang("Java", sc.parallelize(Seq(WikipediaArticle("title", "Java Jakarta")))))
    ...
    println(timing)
    langsRanked.foreach(println) // TODO remove after test
    sc.stop()
  }
